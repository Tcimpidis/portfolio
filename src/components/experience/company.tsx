...
export const Company: FC<CompanyProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      const companyResponse = await fetch('http://127.0.0.1:3000/portfolio/api/company-data', {
        method: "get",
      });
      const companyData = await companyResponse.json() as CompanyMap;
      const projectResponse = await fetch('http://127.0.0.1:3000/portfolio/api/project-data', {
        method: "get",
      });
